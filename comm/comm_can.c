...
/**
 * Transmit CAN packet with extended ID. 发送带有扩展ID的CAN数据包
 *
 * @param id
 * EID
 *
...
/**
 * Check if a VESC on the CAN-bus responds.检查CAN总线上的VESC是否响应
 *
 * @param controller_id
 * The ID of the VESC.
 *
...
/**
 * Detect and apply FOC settings.检测并应用FOC设置
 *
 * @param controller_id
 * The ID of the VESC.
 *
...
/**
 * Update current limits on VESC on CAN-bus.更新CAN总线上VESC的电流限制
 *
 * @param controller_id
 * ID of the VESC.
 *
...
/**
 * Update input current limits on VESC on CAN-bus.更新CAN总线上VESC的输入电流限制
 *
 * @param controller_id
 * ID of the VESC.
 *
...
/**
 * Update FOC ERPM settings on VESC on CAN-bus.在CAN总线上更新VESC上的FOC ERPM设置
 *
 * @param controller_id
 * ID of the VESC.
 *
...
/**
 * Get status message by index.按索引获取状态消息
 *
 * @param index
 * Index in the array
 *
...
/**
 * Get status message by id.按id获取状态消息
 *
 * @param id
 ...
 * no frames are available NULL is returned.从RX缓冲区获取帧。接口是要读取的CAN接口。如果没有可用帧，返回NULL
 *
...
void comm_can_send_status2(uint8_t id, bool replace) {  //BMS
	int32_t send_index = 0;
	uint8_t buffer[8];
	buffer_append_int32(buffer, (int32_t)(mc_interface_get_amp_hours(false) * 1e4), &send_index);  //查询累计安培小时（Ah）消耗 的功能 
...
void comm_can_send_status3(uint8_t id, bool replace) {
	int32_t send_index = 0;
	uint8_t buffer[8];
	buffer_append_int32(buffer, (int32_t)(mc_interface_get_watt_hours(false) * 1e4), &send_index);  //BMS
...
void comm_can_send_status4(uint8_t id, bool replace) {
	int32_t send_index = 0;
	uint8_t buffer[8];
	buffer_append_int16(buffer, (int16_t)(mc_interface_temp_fet_filtered() * 1e1), &send_index);   //MOSFET温度
...
static THD_FUNCTION(cancom_status_thread, arg) {
    ...
    for(;;) {
        ...
        if (conf->can_mode == CAN_MODE_VESC) {
            ...
            //  【新增】发送状态 7           
            comm_can_transmit_status_7(conf->controller_id);
        }
        ...
        // 这里的 sleep_time 决定了发送频率
        // 比如 VESC Tool 里设置 50Hz，这里就是 20ms
        systime_t sleep_time = CH_CFG_ST_FREQUENCY / conf->can_status_rate_1;
        if (sleep_time == 0) {
            sleep_time = 1;
        }
...
static void decode_msg(uint32_t eid, uint8_t *data8, int len, bool is_replaced) {
	...
	if (id == 255 || id == id1 || id == id2) {
        ...
        // 自定义版本号查询
        // ============================================================
		case CAN_PACKET_GET_PRODUCT_INFO: { // ID: 100 (定义在datatypes.h里)
			int32_t send_index = 0;
			uint8_t buffer[8];
            
            // 1. 获取发送者的ID (飞控的ID)
            // 约定：飞控发送查询指令时，payload的第0个字节必须是飞控自己的CAN ID
            uint8_t sender_id = 0;
            if (len > 0) {
                sender_id = data8[0];
            } else {
                // 如果飞控没发ID，为了保险，可以不回或者回给广播地址(255)
                // 这里假设飞控会发
                sender_id = 255; 
            }

            // 2. 填充版本信息 (宏定义在 conf_general.h)
			buffer[send_index++] = PRODUCT_VERSION_MAJOR;
			buffer[send_index++] = PRODUCT_VERSION_MINOR;
			buffer[send_index++] = PRODUCT_VERSION_PATCH;
            ...
            // 4. 发送回包
            // 目标ID: sender_id (飞控)
            // 命令ID: 保持用 100 (CAN_PACKET_GET_PRODUCT_INFO)，飞控收到100就知道是回复
			comm_can_transmit_eid_replace(sender_id |
					((uint32_t)CAN_PACKET_GET_PRODUCT_INFO << 8), buffer, send_index, true, 0);
		} break;       
...
// ============================================================
// 【自定义】发送 Status 7 (ID 69)
// 内容: RPM(Scale 1), Current(Scale 10), Voltage(Scale 10), Temp(Scale 10)
// ============================================================
void comm_can_transmit_status_7(uint8_t controller_id) {
    int32_t send_index = 0;
    uint8_t buffer[8]; // 必须是8字节

    // 1. 获取数据
    // 【修正】RPM Scale = 10。
    // 原理：将 ERPM 除以 10 放入 int16。最大支持 +/- 327,670 ERPM。
    int16_t rpm = (int16_t)(mc_interface_get_rpm() / 10.0);

    // Current: 乘以10 (例如 50.5A -> 505)
    int16_t current = (int16_t)(mc_interface_get_tot_current_filtered() * 10.0);

    // Voltage: 乘以10 (例如 50.2V -> 502)
    int16_t voltage = (int16_t)(mc_interface_get_input_voltage_filtered() * 10.0);

    // MosTemp: 乘以10 (例如 35.5度 -> 355)
    int16_t temp_fet = (int16_t)(mc_interface_temp_fet_filtered() * 10.0);
								 
    // 2. 填充 Buffer (大端模式 Big Endian)
    buffer_append_int16(buffer, rpm, &send_index);      // Byte 0-1
    buffer_append_int16(buffer, current, &send_index);  // Byte 2-3
    buffer_append_int16(buffer, voltage, &send_index);  // Byte 4-5
    buffer_append_int16(buffer, temp_fet, &send_index); // Byte 6-7

    // 3. 发送
    // 使用 controller_id | (69 << 8) 组合出扩展帧 ID
    comm_can_transmit_eid_replace(controller_id |
            ((uint32_t)CAN_PACKET_STATUS_7 << 8), buffer, send_index, true, 0);
}
