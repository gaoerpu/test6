...
bool commands_is_initialized(void);  //是否初始化完成
void commands_send_packet(unsigned char *data, unsigned int len);
void commands_send_packet_can_last(unsigned char *data, unsigned int len);
void commands_send_packet_nrf(unsigned char *data, unsigned int len);   //2.4G无线
...
void commands_process_packet(unsigned char *data, unsigned int len,   //处理数据包
...
bool commands_set_app_data_handler(void(*func)(unsigned char *data, unsigned int len));  //处理手机APP发来的数据
...
void commands_send_mcconf(COMM_PACKET_ID packet_id, mc_configuration* mcconf,  //发送mc配置参数，就是上位机右侧的3个M
    void(*reply_func)(unsigned char* data, unsigned int len));
