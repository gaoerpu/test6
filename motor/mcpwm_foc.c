...
#define _GNU_SOURCE
#endif
//该文件实现了完整的FOC控制算法，包括电流控制、速度控制和位置控制等功能
#include "mcpwm_foc.h"
...
//#define M_MOTOR：对于单电机编译，扩展为&M_MOTOR_1。
//对于双电机，根据is_second_motor的不同，扩展为&m_motor_1或_2。
#ifdef HW_HAS_DUAL_MOTORS
...
static void timer_reinit(int f_zv) {
	...
	TIM_TimeBaseStructure.TIM_Period = (SYSTEM_CORE_CLOCK / f_zv);  // 168000000 / 30000 = 5600
...
void mcpwm_foc_set_configuration(mc_configuration *configuration) {
	...
	uint32_t top = SYSTEM_CORE_CLOCK / (int)configuration->foc_f_zv;   // 168000000 / 30000 = 5600
	if (TIM1->ARR != top) {
#ifdef HW_HAS_DUAL_MOTORS
		m_motor_1.m_control_mode = CONTROL_MODE_NONE;
		m_motor_1.m_state = MC_STATE_OFF;
...
/**
 * Switch off all FETs.关闭所有场效应晶体管。
 */
 ...
 */
float mcpwm_foc_get_switching_frequency_now(void) {       //PWM 切换频率
 ...
 */
float mcpwm_foc_get_sampling_frequency_now(void) {          //电流采样频率
 ...
 */
void mcpwm_foc_set_current_off_delay(float delay_sec) {  //设置电流关闭延迟。防止电流控制器关闭调制
...
float mcpwm_foc_get_rpm(void) {
	return RADPS2RPM_f(get_motor_now()->m_pll_speed);  //锁相环估计
	//	return get_motor_now()->m_speed_est_fast * RADPS2RPM_f;
}

/**
 * Same as above, but uses the fast and noisier estimator.与上述相同，但使用快速和噪声更大的估计器。
 */
...
/**
 * Same as above, but uses the faster and noisier estimator.与上述相同，但使用更快、更嘈杂的估计器。
 */
 ...
 * (negative) current. This is the q-axis current which produces torque.  获取电流
 *
 * @return
 * The motor current.
 */
 ...
 * represent whether the motor is drawing (positive) or generating
 * (negative) current. This is the q-axis current which produces torque.  获取滤波后的电流
 *
 * @return
...
/**
 * Get the magnitude of the motor current unbalance 获取电机电流不平衡的大小
 *
 * @return
...
/**
 * Get the magnitude of the motor voltage. 获取DQ轴电压的大小。
 *
 * @return
 ...
 */
int mcpwm_foc_set_tachometer_value(int steps) {  //设置电机旋转的步数。该数字有符号，当电机向后旋转时变为负数
 ...
 * be this number divided by (3 * MOTOR_POLE_NUMBER).
 */
int mcpwm_foc_get_tachometer_abs_value(bool reset) {  //读取电机旋转的绝对步数
...
int mcpwm_foc_measure_resistance(float current, int samples, bool stop_after, float *resistance) {
	...
	volatile motor_all_state_t *motor = get_motor_now(); //先检测是一个电机还是两个电机
	int fault = FAULT_CODE_NONE;

	motor->m_phase_override = true;//强制使用覆盖相位
	...
	// Disable timeout 禁用超时：保存原配置VESC 固件通常有一个安全超时机制。
	//如果在一定时间内没有接收到控制指令（如来自遥控器或上位机），电机将自动停止，以防止失控。
	//在进行电阻测量时，这个过程是自动的，不需要外部指令。因此，函数会先保存当前的超时配置，
	//然后设置一个很长的超时时间（60 秒）并禁用急停开关功能，以确保测量过程不会被安全机制意外中断。
	//测量结束后，会恢复原始的超时配置。
	systime_t tout = timeout_get_timeout_msec();
	float tout_c = timeout_get_brake_current();
	KILL_SW_MODE tout_ksw = timeout_get_kill_sw_mode();
	timeout_reset();
	timeout_configure(60000, 0.0, KILL_SW_MODE_DISABLED);//设 60s 超时 + 0A 刹车 + 禁用 kill switch

	// Ramp up the current slowly 缓慢地增加电流
	while (fabsf(motor->m_iq_set - current) > 0.001) {
		utils_step_towards((float*)&motor->m_iq_set, current, fabsf(current) / 200.0);
		fault = mc_interface_get_fault();// 检查故障（过流/过压等）
		if (fault != FAULT_CODE_NONE) {   // 有任何故障代码都会应急停止
			motor->m_id_set = 0.0;
			motor->m_iq_set = 0.0;
			motor->m_phase_override = false;
			motor->m_control_mode = CONTROL_MODE_NONE;
		...
		chThdSleepMilliseconds(1);   //1ms （ChibiOS RTOS 延时）
	}

	// Wait for the current to rise and the motor to lock.
	//等待电流上升，电机锁定。 50ms：电流稳 + 机械惯性锁死（转子不动）
	...
	const float current_avg = motor->m_samples.avg_current_tot / (float)motor->m_samples.sample_num;  //累加的电流值除数量
	const float voltage_avg = motor->m_samples.avg_voltage_tot / (float)motor->m_samples.sample_num;  //累加的电压值除数量
 ...
 * @return
 * The fault code
 */ //核心：用 HFI（高频信号注入）+ FFT，从 dq 轴的电流响应里反推 Ld、Lq。
int mcpwm_foc_measure_inductance(float duty, int samples, float *curr, float *ld_lq_diff, float *inductance) {  //测量电感：用高频注入法测量 
	volatile motor_all_state_t *motor = get_motor_now();
	int fault = FAULT_CODE_NONE;
	...
	for (int i = 0;i < (samples / 10);i++) {
		...
		// FFT处理电压缓冲区：提取0次谐波（直流分量）和2次谐波（凸极效应导致）
	...
	float ind_scale_factor = 0.9;  //工程系数  工程修正系数（避免电感高估导致观测器不稳定）  
	...
	if (ld_lq_diff) {
		*ld_lq_diff = (ld_lq_diff_sum / iterations) * 1e6 * ind_scale_factor;  // 转微亨+修正  
	}
 ...
 * The fault code
 */
int mcpwm_foc_measure_inductance_current(float curr_goal, int samples, float *curr, float *ld_lq_diff, float *inductance) {  //测量电感，区别上一个：这个是电流导向的优化版，更安全
	int fault = FAULT_CODE_NONE;
	float duty_last = 0.0;
...
bool mcpwm_foc_beep(float freq, float time, float voltage) {  //通过电机线圈产生蜂鸣声（beep）
...
void mcpwm_foc_adc_int_handler(void *p, uint32_t flags) {  //ADC中断处理程序
	(void)p;
	(void)flags;
...
#ifdef HW_HAS_DUAL_MOTORS   //高端三相电流采样
...
#define SHUNT_PICK_THR 900
	...
	// Use the best current samples depending on the modulation state.根据调制状态使用最佳电流样本。
#ifdef HW_HAS_3_SHUNTS  //3电阻采样代码
	if (conf_now->foc_current_sample_mode == FOC_CURRENT_SAMPLE_MODE_HIGH_CURRENT) {
		full_clarke = false;

		// High current sampling mode. Choose the lower currents to derive the highest one找出电流绝对值最大的那相，不直接测它，用另外两相的实测值算出来
		// in order to be able to measure higher currents.能避免直接测量最大或者最小的那一相电流，从而减少电流采样误差
...
//				curr1 = -(curr0 + curr2);
	...
	if (motor_now->m_state == MC_STATE_RUNNING) {   			//电机运行的操作 
		if (full_clarke) {
			// Full Clarke Transform   完整的 clarke变换  如果是三电阻采样，就是用完整的clarke变换
			...
			// Clarke transform assuming balanced currents  假设电流平衡的 clarke变换   ia + ib + ic = 0
...
//		run_fw(motor_now, dt);
		...
		control_current(motor_now, dt);  //电流环控制
	} else {
		...
		// spike.
		motor_now->m_phase_now_observer += motor_now->m_pll_speed * dt * conf_now->foc_observer_offset;  // 补偿观测器延迟 补偿公式：Δθ = ω × Δt × (0.5 + offset)
		utils_norm_angle_rad((float*)&motor_now->m_phase_now_observer);
...
//主要作用是在1kHz定时器线程中执行FOC控制的非实时更新任务，包括温度补偿参数、开环启动逻辑、观测器增益缩放、电阻估计和电流偏移校准。
static void timer_update(motor_all_state_t *motor, float dt) {
	foc_run_fw(motor, dt);

	const mc_configuration *conf_now = motor->m_conf;

	// Calculate temperature-compensated parameters here                 0.00386 %/°C  铜的温度系数
...
static void control_current(motor_all_state_t *motor, float dt) {
	...
	// Low passed currents are used for less time critical parts, not for the feedback低通电流用于时间要求较低的部件，而不是用于反馈
	...
	if (conf_now->foc_temp_comp) {
		ki = motor->m_current_ki_temp_comp;   // 温度补偿
	}

	state_m->vd_int += Ierr_d * (ki * d_gain_scale * dt);  // 积分项
	...
	// vq = Rs*iq   +   Lq*diq/dt +  ωe*id*Ld     + ωe*ψm         电流环解耦
	float dec_vd = 0.0;
	float dec_vq = 0.0;
	float dec_bemf = 0.0;
...
/*
这里解释一下为什么是vd=vd - dec_vd，而不是vd = vd + dec_vd
根据PMSM的数学模型，d轴电压方程为：
FOC中PI输出的：ud = Rs*id + Ld*did/dt 
实际电机模型的：vd = Rs*id + Ld*did/dt - ωe*iq*Lq
要让上面两个公式对齐，需要把PI输出的ud减去交叉耦合项ωe*iq*Lq，即：
vd = ud - ωe*iq*Lq

从逻辑上来解释：
假设电机处于稳态，电流恒定，即did/dt=0，那么d轴电压方程简化为：
vd = Rs*id - ωe*iq*Lq
vd = 0 - ωe*iq*Lq
要维持恒定的d轴电流=0，所以vd = 0 - ωe*iq*Lq<0
即vd一直为负，如果此时用+ωe*iq*Lq，这时的ud=0了，就不是负的了，所以只能用-ωe*iq*Lq
PI输出的就是ud=0，而实际的电机模型需要的vd是负的，所以只能用-ωe*iq*Lq。

*/
	// Calculate the max length of the voltage space vector without overmodulation.计算无过调制的电压空间矢量的最大长度。
	// Is simply 1/sqrt(3) * v_bus. See https://microchipdeveloper.com/mct5001:start. Adds margin with max_duty.
	float max_v_mag = ONE_BY_SQRT3 * max_duty * state_m->v_bus * conf_now->foc_overmod_factor;   //foc_overmod_factor=1
    // 0.577限制的是 αβ 平面中的“可实现电压空间矢量的线性区半径”  线性区最大电压矢量幅值
	// Saturation and anti-windup. Notice that the d-axis has priority as it controls field
	// weakening and the efficiency.饱和和抗饱和。请注意，d轴具有优先级，因为它控制字段削弱和效率。
	...
	//state_m->vd_int += (state_m->vd - vd_presat);   vd² + vq² ≤ max_v_mag²。

	float max_vq = sqrtf(SQ(max_v_mag) - SQ(state_m->vd));
	//float vq_presat = state_m->vq;
	utils_truncate_number_abs((float*)&state_m->vq, max_vq);
	utils_truncate_number_abs((float*)&state_m->vq_int, max_vq);   // 积分项限幅，如果不限制积分项，电压饱和时积分会继续累积，等饱和解除后系统反应会很慢（积分饱和问题）
	...
	//    voltage_normalize = 1/(2/3*V_bus)      0.666是dq电压的基准值
    ...
    //  0.577Udc/0.666Udc=0.866  在不进入过调制（overmodulation）的前提下，SVPWM 所能达到的最大“调制度”,0.866相当于是把0.577归一化了一下
	//					0.866是线性区最大的mod幅值						
...
static void update_valpha_vbeta(motor_all_state_t *motor, float mod_alpha, float mod_beta) {
	...
	//策略是根据电流方向判断误差方向，然后修正占空比。   死区影响的是低速电流波形的平滑
	float s = state_m->phase_sin;
	float c = state_m->phase_cos;
