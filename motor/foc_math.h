...
// Functions
void foc_observer_update(float v_alpha, float v_beta, float i_alpha, float i_beta,  	//磁链观测器
		float dt, observer_state *state, float *phase, motor_all_state_t *motor);
void foc_pll_run(float phase, float dt, float *phase_var,  			//锁相环
...
void foc_run_fw(motor_all_state_t *motor, float dt); 				//弱磁控制
