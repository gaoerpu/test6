...
bool mc_interface_wait_for_motor_release(float timeout) {  //主要负责等待电机释放的阻塞检查
 ...
 */
float mc_interface_get_amp_hours(bool reset) {  //查询累计安培小时（Ah）消耗 的功能
 ...
 */
float mc_interface_get_amp_hours_charged(bool reset) {   //查询累计安培小时（Ah）充电量的功能
 ...
 */
float mc_interface_get_watt_hours(bool reset) {  //查询累计瓦特小时（Wh）消耗的功能
...
float mc_interface_read_reset_avg_input_current(void) {  //主要负责读取并重置平均输入电流的查询功能
...
/**
 * Read and reset the average direct axis motor current. (FOC only) 读取并重置平均d轴电机电流
 *
 * @return
...
/**
 * Set odometer value in meters.以米为单位设置里程表值
 *
...
void mc_interface_release_motor_override_both(void) {  //负责释放两个电机的覆盖状态的功能
...
bool mc_interface_wait_for_motor_release_both(float timeout) {  //主要负责等待两个电机同时释放的阻塞检查
	int motor_last = mc_interface_get_motor_thread();
...
void mc_interface_set_current_off_delay(float delay_sec) {  //要负责设置电流关闭延迟的功能
	if (mc_interface_try_input()) {
		return;
	}
 ...
 *
 */
int mc_interface_try_input(void) {   //主要负责校验输入指令的有效性的功能
