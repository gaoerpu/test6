...
#pragma GCC optimize ("Os")
           //该文件实现通过PPM信号控制电机，包含PPM信号解码、输入值处理和电机控制等功能
#include "app.h"
...
static THD_FUNCTION(ppm_thread, arg) {
	...
	for(;;) {
		...
		case PPM_CTRL_TYPE_PID_POSITION_360:
			input_val = servo_val;  
			servo_val += 1.0;     //这个是把-1到1的值，转换到0到1之间
			servo_val /= 2.0;
			break;
		...
		default:
			...
			if (servo_ms < config.pulse_center) {
				servo_val = utils_map(servo_ms, config.pulse_start,    //线性映射函数
...
//		}
		...
		if (ramp_time > 0.01) {
			...
			utils_step_towards(&servo_val_ramp, servo_val, ramp_step);   //斜率限制/平滑
