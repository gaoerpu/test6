...
int main(void) {
	...
	HW_EARLY_INIT();  //空：空间给用户自定义的早期初始化
	...
	chThdSleepMilliseconds(100);  //延时函数：100ms

	mempools_init();   //内存池初始化
	events_init();     //事件系统初始化，比如说你设置一个指令，然后事件会进行记录，后期可以查看
	timer_init(); 	   // Initialize timer here to allow I2C in hw_init
	hw_init_gpio();    //IO口初始化
	LED_RED_OFF();
	LED_GREEN_OFF();

	conf_general_init();  //配置参数初始化

	if (flash_helper_verify_flash_memory() == FAULT_CODE_FLASH_CORRUPTION)	{  //flash完整性检查
	...
	ledpwm_init();   		//呼吸灯LED初始化
	mc_interface_init();	//电机控制接口初始化

	commands_init();		//执行命令初始化，就是执行上位机的命令

#if COMM_USE_USB
	comm_usb_init();    	//上位机USB通信初始化
#endif
...
#ifdef HW_HAS_PERMANENT_NRF         //永久NRF：2.4G无线模块，主要用于无线遥控方面
	...
	// Threads  初始化线程
	chThdCreateStatic(led_thread_wa, sizeof(led_thread_wa), NORMALPRIO, led_thread, NULL);		//led线程
	chThdCreateStatic(periodic_thread_wa, sizeof(periodic_thread_wa), NORMALPRIO, periodic_thread, NULL);  //周期性线程
	chThdCreateStatic(flash_integrity_check_thread_wa, sizeof(flash_integrity_check_thread_wa), LOWPRIO, flash_integrity_check_thread, NULL);  //flash线程

	timeout_init();  //看门狗超时保护初始化
	timeout_configure(appconf->timeout_msec, appconf->timeout_brake_current, appconf->kill_sw_mode);  //看门狗配置

#if HAS_BLACKMAGIC
	bm_init();
#endif

	shutdown_init();   //关机模块初始化

	imu_reset_orientation();  //imu初始化

	chThdSleepMilliseconds(500);  //延迟
	m_init_done = true;  //系统初始化完成标志位置1
	...
	for(;;) {
		chThdSleepMilliseconds(10);  //进入死循环，不断执行前面的三个线程
	}
}
