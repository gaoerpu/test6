...
//该文件实现舵机信号解码功能，包含输入捕获、脉冲宽度测量和位置计算等操作			 
// Settings
#define SERVO_NUM				1    // 只处理 1 路 PWM 信号
#define TIMER_FREQ				1000000   // 定时器频率 1MHz (即 1 tick = 1 微秒)
...
static void icuwidthcb(ICUDriver *icup) {
	...
	if (len > len_set) {
		if (len < (len_set * 1.5)) {   // 如果超得不多 (比如 2.1ms)，就当成满油门处理 
			len = len_set; 
		} else {
			// Too long pulse. Most likely something is wrong.
			len = -1.0;  // 如果超得太多 (比如 5ms)，认为是错误信号/干扰，标记为无效
		}
	} else if (len < 0.0) {
...
#endif
		last_update_time = chVTGetSystemTimeX();  // 更新系统时间戳，证明“我还活着” (用于超时保护)
...
static ICUConfig icucfg = {
		...
		NULL,
		NULL,
		HW_ICU_CHANNEL,  // 硬件通道 (在 hw_xxx.h 中定义，比如 TIM3_CH1)
		0
};
...
void servodec_init(void (*d_func)(void)) {
	...
	icuStartCapture(&HW_ICU_DEV);   // 开始捕获
...
void servodec_stop(void) {
	if (is_running) {
		...
		icuStop(&HW_ICU_DEV);
		palSetPadMode(HW_ICU_GPIO, HW_ICU_PIN, PAL_MODE_INPUT);// 恢复引脚为普通输入，避免干扰
		pulse_start = 1.0;
		pulse_end = 2.0;
